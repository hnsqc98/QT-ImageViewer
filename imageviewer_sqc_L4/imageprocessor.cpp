...
int ImageProcessor::nolinearflag;
...
QImage ImageProcessor::gaussFilter(const QImage& img,int s,double xigema)
{
    ...
    for (int i=0;i<s;++i)
    {
        for(int j=0;j<s;++j)
        {
            int xx=abs(i-center);
            int yy=abs(j-center);
            double res=exp(-(xx*xx/2.0/xigema/xigema+yy*yy/2.0/xigema/xigema));
            data.push_back(res);
        }
    }
    ...
    qDebug()<<data;
    modeFlag=1; //重复模式
    return filtercolor(img,data,s);
}

QImage ImageProcessor::bilateralFilter(const QImage& img,int s,double xigemas,double xigemac)
{
    std::vector<double> bdata;
    int center=s/2;

    for (int i=0;i<255;++i)  //创建灰度乘方表
    {
        double tmp=exp(-i*i/2.0/xigemac/xigemac);
        bdata.push_back(tmp);
    }
        ...
        for (int j=0;j<h;++j)
        {
            ...
            for (int q=0;q<s;++q)
            {
                for(int p=0;p<s;++p)
                {
                    int xx=abs(q-center);
                    int yy=abs(p-center);
                    double res=exp(-(xx*xx/2.0/xigemas/xigemas+yy*yy/2.0/xigemas/xigemas));

                    int minus=qGray(patch.pixel(q,p))-qGray(patch.pixel(center,center));
                    double fres=res*bdata[abs(minus)];
                    data.push_back(fres);
                }
            }
           ...
           // qDebug()<<data;
            ...
            for (int k=0;k<data.size();++k)
            {
                sum+=data[k]*qGray(patch.pixel(k/s,k%s));
            }
            if(sum<=0) sum=0;
            if(sum>=255) sum=255;
            ret.setPixel(i,j,qRgb(sum,sum,sum));
...
QImage ImageProcessor::nolinearFilter(const QImage& img,int s)
{
    //qDebug()<<"median";
    QImage ret(img);
    int w=img.width();
    ...
    for (int i=0;i<w;++i)
    {
        for (int j=0;j<h;++j)
        {
            ...
            for (int k=0;k<s*s;++k)
            {
               data<<qGray(patch.pixel(k/s,k%s));
            }
            std::sort(data.begin(),data.end());

            int res;
            if (nolinearflag==1) res=data[s*s/2];   //中值
            else if (nolinearflag==2) res=data[0];   //腐蚀
            else if (nolinearflag==3) res=data[s*s-1];  //膨胀
